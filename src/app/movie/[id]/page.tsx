...
export default function MoviePage() {
  ...
  useEffect(() => {
    ...
    setIsInMyList(myList.some((m: { id: string }) => m.id === id));
  ...
  const handleAddToMyList = () => {
    ...
    if (isInMyList) {
      const updatedList = myList.filter((m: { id: string }) => m.id !== id);
  ...
  return (
    <div className="relative min-h-screen bg-black pb-20">
      ...
      <button 
        onClick={() => router.back()} 
        className="absolute top-20 left-4 z-20 flex items-center space-x-2 text-white md:left-10"
      >
